...
import static com.devdmitryn.userofthedaybot.controller.util.TextMessage.LOSER_STATISTIC;

@Component
@Scope("prototype")
public class LoserStatisticCommand extends Command {
    ...
    public void execute(Message receivedMessage) throws TelegramApiException {
        ...
        StringBuffer text = new StringBuffer(LOSER_STATISTIC);

        for (UserInChat userInChat : usersInChat) {
            text.append(userInChat.getUser().getNotNullName() + " - " + userInChat.getLoserCounter() + ",\n");
        }
